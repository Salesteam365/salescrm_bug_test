...
<style>
   .btnbackgroundcustom{
      background-color:rgba(35,0,130,0.9);
      color:white;
   }
</style>
<script>
   ...
   $.ajax({
      ...
      success: function (response) {
         ...
         if (fullData.length === 0) {
            ...
            $("#putNotiFication").html('<li class="nav-item">No notifications available</li>');
         } else {
            $.each(fullData, function (index, data) {
               if (k == 0) {
                  var notiId = data['id'];
      ...
      success: function (response) {
         ...
         $.each(fullData, function (index, data) {
            if (data['id'] != "") {
               ...
               var text = '<li class="nav-item" id="notili"style="width:100%;" >' +
                  '<a href="' + data['url'] + '" class="nav-link" style="padding: 0px 0px 0px 5px;" >' +
                  '<div style="padding:0px; font-size:13px;">' + data['subject'] + '<div class="row"><div class="col-md-6"><label style="padding:0px; font-size:11px;">' + data['owner'] + '</label></div><div class="col-md-6"><label style="padding:0px;font-size:11px;float: right;margin-right: 7px;">' + data['created_date'] + '</label></div></div></div></a></li>' +
                  '<li style="width:100%;" ><div class="dropdown-divider"></div></li>';
...
$(".btn-info").addClass('btnbackgroundcustom');
$(".btnbackgroundcustom").removeClass('btn-info');
$("thead").addClass('btnbackgroundcustom');

// $(document).on('keydown',function(e){
//    if(e.keyCode == 16){
//       $(".content-wrapper").each(function(){
//          $(this).css({'background':'rgba(0,0,0,0.9)','color':'white'});
//       });
//       $(".card").each(function(){
//          $(this).css({'background':'rgba(0,0,0,0.9)','color':'white'});
//       })
