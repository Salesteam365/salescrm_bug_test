...
<div class="content-wrapper">
   ...
   <form class="form-horizontal"  id="form" method="post" enctype = "multipart/form-data">
      <div class="form-proforma">
         <div class="container">
            <div class="row">
               <input type="hidden" value="<?=$this->uri->segment(2);?>" name="po_id">
               ...
               <input type="hidden"  name="subscr_type" id="subscr-type" value="<?php if(isset($record['subscr_type']) ){ echo $record['subscr_type']; }  ?>">
                     ...
                     <div class="proforma-table">
                        <table class="table table-responsive-lg" width="100%" id="add_new_line" >
                           ...
                           <?php
                              ...
                              for($pr=0; $pr<count($proName); $pr++){ 
                                 if($action['data'] == 'add') {
                                    ...
                                    $subscrcount=1;
                                    if($record['subscr_type']=='yom' || $record['subscr_type']=='mom'){
                                       $subscrcount=12;
                                    }
                                 }
                                 ...
                                 if (!in_array($proName[$pr], $proArr) || !in_array($proDummyId[$pr], $proArrId))
                              	{
                                    ...
                                    if($record['subscr_type']=='yom' || $record['subscr_type']=='mom'){
                                       $subscrcount=12;
                                        ...
                                        $pricearrr=explode("<br>",$subscr_po['product_name']);
                                        foreach ($pricearrr as $productName) {
                                            if (!array_key_exists($productName, $groupedProducts)) {
                                                $groupedProducts[$productName] = array();
                                              }
                                          $groupedProducts[$productName][] = $productName;
                                        }
                                        $groupedProducts = array_values($groupedProducts);
                                        $subscrcount =  count($groupedProducts[$pr]);
                                        $pricestringtoarr = explode("<br>",$subscr_po['unit_price']);
                                        $pricearr = array_slice($pricestringtoarr, $priceindex, count($groupedProducts[$pr]));
                                        $discountstringtoarr = explode("<br>",$subscr_po['discount']);
                                        $discountarr = array_slice($discountstringtoarr, $priceindex, count($groupedProducts[$pr]));
                                  
                                    }
                                  
                                   
                                     for($i=0; $i<$subscrcount; $i++){
                           ...
                           <tr class="removCL<?=$rw;?>">
                              ...
                              <td><input type="text" onkeyup="calculate_pro_price()" name="unit_price[]" id="price<?=$rw;?>" class="form-control start  price_float"  placeholder="rate" value="<?=$pricearr[$i]?>"  ><span id="unit_price_error"></span></td>
                              <td><input type="text" onkeyup="calculate_pro_price()" name="discount_price[]" id="disc<?=$rw;?>" class="form-control price_float"  placeholder="Discount Price" minlength="0" value="<?php if(isset($discountarr[$i]) && $action['data']=='update' ){ echo $discountarr[$i]; }else{ echo "0"; } ?>"></td>
                              ...
                              <td>
                                 <input type="text"  name="total[]" class="form-control" value="<?=$total[$pr];?>" >
                              ...
                              <td style="cursor:pointer;"><i  class="fa-solid fa-xmark rowdelete"></i></td>
                           </tr>
                           ...
                           <?php $rw++; $priceindex+=count($groupedProducts[$pr]);}  else{for($i=0; $i<$subscrcount; $i++){ ?>
                              <tr class="removCL<?=$rw;?>">
...
function save(){
	if(checkValidationWithClass('form')==1){
			...
			if(save_method == 'add') {
				url = "<?= site_url('purchaseorders/subpo_create')?>";
			} else {
				url = "<?= site_url('purchaseorders/subpo_update')?>";
			}
			$.ajax({
				url : url,
				type: "POST",
