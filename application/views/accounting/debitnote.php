 ...
 <div class="modal fade" id="linkedinvoiceform" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add new Debit Note</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form id="debitNoteForm" method="post" action="<?php echo base_url('add-debitnote'); ?>">
  ...
  <div class="content-wrapper" style="background-color:rgba(240,240,246,0.3);overflow-x:clip;">
                    ...
                    <div class="col-lg-1"></div>
                        ...
                        <div class="col-lg-6">
                          <div class="refresh_button float-right">
                            ...
                            <?php if($this->session->userdata('type')=='admin'){ ?>
                            <a href='Export_data/export_debitnote' class="p-0" ><button class="btncorner">Export Data</button></a>
                            <?php } ?>
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                              <!-- <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="<?php echo base_url('add-debitnote');?>">Add New Debit Note</a></div> -->
                              <div class="dropdown-item" id ="addnewdebitnote"><a style="color:black; padding-left:0px;" href="javascript:void(0);">Add New Debit Note</a></div>
    ...
    <!-- filter box starts-->
          ...
          <div class="card-body">
            <!-- <h4 style="margin-bottom:18px;">Filter</h4> -->
    ...
    <!-- filter box ends -->
         <div class="accordion mx-3" id="faq" >
            <div class="quoteaccordion">
               <div class="quoteacc_head" id="faqhead1" data-toggle="collapse" data-target="#faq1" aria-expanded="false" aria-controls="faq1"> <a href="#" class="btn btn-header-link" ><i class="fas fa-file-alt"></i> Debit Note Summary</a>
               </div>
            ...
            <div class="quoteaccordion">
               <div class="quoteacc_head" id="faqhead2" data-toggle="collapse" data-target="#faq2" aria-expanded="false" aria-controls="faq2"> <a href="#" class="btn btn-header-link" ><i class="fas fa-file-alt"></i> Debit Note Graph</a>
               </div>
...
<div class="wrapper card p-4" style="border-radius:12px;border:none;box-shadow:0px;">
            <div class="card-header mb-2"><b style="font-size:21px;">Debit Note</b> <button type="button" id="btnshowhide" class="btn btn-outline-secondary" data-toggle="modal" data-target="#exampleModal" style="">
                   <i class="fa fa-table" aria-hidden="true" style="color:purple;"></i>&nbsp;
           ...
           <!-- model end -->
                    <thead>
                      <tr>
                        ...
                        <?php endif; ?>
                        <th class="th-sm" style="" >Debit Date</th>
                        <th class="th-sm" style="">Debit No.</th>
                        <th class="th-sm">Debit Amount </th>
                        <th class="th-sm">Invice No.</th>
...
////////////////////////////////////////////// it triggers automatically when page load (starts)///////////////////////////////////////////////////////////
    ...
    $(this).change(function(){
      var filtername = $(this).attr('name');
       ...
       $.ajax({
        url:'<?php echo site_url('Accounting/getdebitnotedata');?>',
        method:'post',
        data:formdata,
  ...
  //     $("#debitNoteForm").submit();
  //   });

    $("#saveChangesBtn").click(function() {
  
        event.preventDefault();
        var formData = $("#debitNoteForm").serialize();
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Accounting/add_debitnote');?>",
            data: formData,
            dataType: 'json', 
            success: function(response) {
                if (response.success) {
                  var invoiceId = response.invoice_id;
                    var redirectUrl = "<?php echo base_url('add-debitnote/');?>" + invoiceId;
   ...
   table = $('#ajax_datatable').DataTable({
      ...
      "ajax": {
          ...
          "dataSrc": function(json) {
           ...
           $('#totalRecords').text("Debit Notes : " + json.recordsTotal_debit);
       
            $('#totalAmounts').text("Debit Amount : ₹ " + json.Totalamount_debit); 
            return json.data; 
        }
      },
...
function delete_debitnote(id)
  {
      if(confirm('Are you sure delete this data?'))
      {
          // ajax delete data to database
          $.ajax({
              url : "<?= site_url('Accounting/debitnote_delete')?>/"+id,
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
 ...
 function refreshPage()
  {
   ...
   $("#addnewdebitnote").click(function(){
...
<script>
    ...
    $.ajax({
        url:'<?php echo site_url('Accounting/get_debitnote_graph')?>',
        method:'post',
        success:function(response){
        ...
        for (var i = 0; i < response.data.length; i++) {
          expanse_amount.push(parseFloat(response.data[i].subtotal)); 
          xAxisCategories.push(response.data[i].month + "/" + response.data[i].year);
        }
console.log(expanse_amount)

      var options = {
          series: [
              { name: 'Debit Note Amount of month', data: expanse_amount },
          ],
          chart: {
              height: 250,
              type: 'area'
