      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Add  New Delivery Challan </h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form id="deliveryForm" method="post" action="<?php echo base_url('add-deliverychallan'); ?>">
            <div class="form-group">
              <label for="exampleInput">Linked Invoice No.</label>
              <input type="text" class="form-control" name="invoicenum" id="linkedinvoicenum" placeholder="Enter linked  Delivery Challan number">
  ...
  <div class="content-wrapper" style="background-color:rgba(240,240,246,0.3);overflow-x:clip;">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Delivery Challan</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
                      ...
                      <div class="col-lg-6">
                        <div class="refresh_button float-right">
                            ...
                            <button class="btnstopcorner" onclick="refreshPage()"><i class="fas fa-redo-alt"></i></button>
                              <?php if($this->session->userdata('type')=='admin'){ ?>
                              <a href='Export_data/export_deliverychallan' class="p-0" ><button class="btncorner">Export Data</button></a>
                              <?php } ?>
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                <!-- <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="<?php echo base_url('add-deliverychallan');?>">Add New Delivery Challan</a></div> -->
                                <div class="dropdown-item" id="addnewdeliverychallan"><a style="color:black; padding-left:0px;" href="javascript:void(0);">Add New Delivery Challan</a></div>
    ...
    <!-- filter box ends -->
         <div class="accordion mx-3" id="faq" >
            <div class="quoteaccordion">
               <div class="quoteacc_head" id="faqhead1" data-toggle="collapse" data-target="#faq1" aria-expanded="false" aria-controls="faq1"> <a href="#" class="btn btn-header-link" ><i class="fas fa-file-alt"></i> Delivery Challan Summary</a>
               </div>
            ...
            <div class="quoteaccordion">
               <div class="quoteacc_head" id="faqhead2" data-toggle="collapse" data-target="#faq2" aria-expanded="false" aria-controls="faq2"> <a href="#" class="btn btn-header-link" ><i class="fas fa-file-alt"></i> Delivery Challan Graph</a>
               </div>
...
<div class="wrapper card p-4" style="border-radius:12px;border:none;box-shadow:0px;">
            <div class="card-header mb-2"><b style="font-size:21px;">Delivery Chalan</b> <button type="button" id="btnshowhide" class="btn btn-outline-secondary" data-toggle="modal" data-target="#exampleModal" style="">
                   <i class="fa fa-table" aria-hidden="true" style="color:purple;"></i>&nbsp;
           ...
           <!-- model end -->
                    <thead>
                      <tr>
                        ...
                        <?php endif; ?>
                        <th class="th-sm" style="width:20%;" >Date</th>
                        <th class="th-sm" style="width: 275px;">Delivery Challan</th>
                        <th class="th-sm" style="width: 275px;">Invoice No.</th>
                        <th class="th-sm">Delivered To</th>
                        ...
                        <th class="th-sm">Trade/Legal Name</th>
...
function delete_deliverychallan(id)
  {
      if(confirm('Are you sure delete this data?'))
      {
          ...
          $.ajax({
              url : "<?= site_url('Accounting/deliverychallan_delete')?>/"+id,
              type: "POST",
              dataType: "JSON",
              success: function(data)
              {
               
                $("#common_popupmsg").html('<i class="far fa-check-circle" style="color: #60b963;"></i><br>Delivery Challan information has been deleted.');				
  ...
  <?php if(check_permission_status('Quotations','retrieve_u')==true):?>
    //datatables
      ...
      table = $('#ajax_datatable').DataTable({
        ...
        "order": [],
        "ajax": {
            "url": "<?php echo site_url('Accounting/get_deliverychallan')?>",
            "type": "POST",
            "dataType":"json",
            "data": function (data) {
                ...
                "dataSrc": function(json) {
                    $('#totalRecords').text("Delivery Challans : " + json.recordsTotal);
                    $('#totalAmounts').text("Delivery Challan Amount : ₹ " + json.Totalamount); 
                    return json.data; 
                }
...
$("#addnewdeliverychallan").click(function(){
...
//     });
    ...
    $("#saveChangesBtn").click(function() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('Accounting/add_deliverychallan');?>",
            data: formData,
            dataType: 'json', 
            success: function(response) {
                if (response.success) {
                  var invoiceId = response.invoice_id;
                    var redirectUrl = "<?php echo base_url('add-deliverychallan/');?>" + invoiceId;
...
// -----------------------------------------//
      ////////////////////////////////////// fetch the data of month wise Delivery Challan graph/////////////////////////////////////
 $.ajax({
  url:'<?php echo site_url('Accounting/get_challan_graph')?>',
              method:'post',
              success:function(response){
              ...
              for (var i = 0; i < response.data.length; i++) {
                Challan_amount.push(parseFloat(response.data[i].subtotal)); 
        ...
        var options = {
            series: [
                { name: 'Delivery Challan Amount of month', data: Challan_amount },
            ],
            chart: {
                height: 250,
                type: 'area'
