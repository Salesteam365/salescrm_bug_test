      ...
      <div class="row">
        <section class="col-lg-4 connectedSortable">
          <div class="card direct-chat direct-chat-primary updated_info">
            ...
            <div class="card-body">
              <div class="col-lg-4">
                <b class="text-secondory">Your Logo</b>
              </div>
              <?php if ($this->session->userdata('type') == 'admin') { ?>
                ...
                <?php } elseif ($this->session->userdata('email') === 'demo@sourcecodeautomation.co.in') {
                ?>
                <img src="<?= base_url() . 'uploads/company_logo/demo-jpg.jpg'; ?>" class="img-circle img-fluid" alt="Source Code Automation">
                <?php
                } else { ?>
                  <img src="<?= base_url() . 'uploads/company_logo/default_logo.png'; ?>" class="img-circle img-fluid" alt="User Image">
                <?php } ?>
              <?php } else if ($this->session->userdata('type') == 'standard') { ?>
                <!-- default_logo.png -->
                  <?php $img_url = (!empty($this->session->userdata('company_logo'))) ? $this->session->userdata('company_logo') : 'demo-jpg.jpg' ?>
                  <img src="<?= base_url('uploads/profile_image/'.$img_url); ?>" class="img-circle img-fluid" alt="User Image">
              <?php } ?>
              <div class="direct-chat-messages">
                <h2><?= ucwords($this->session->userdata('name')); ?></h2>
                <p><i class="fas fa-map-marker-alt"></i> <?= ucfirst($this->session->userdata('state')); ?>, <?= ucfirst($this->session->userdata('country')); ?></p>
                <h5><?= ucwords($this->session->userdata('company_name')); ?></h5>
                <h6><?= $this->session->userdata('company_email'); ?></h6>
              </div>
            </div>
          </div>
        </section>
        ...
        <section class="col-lg-8 connectedSortable">
          ...
          <div class="card bg-gradient-primary detail_section">
            <div class="card-tools ml-auto">
              <?php if ($this->session->userdata('type') == 'admin') { ?>
                <a href="<?= base_url(); ?>company?q=update">
                  <button type="button" class="btn bg-info btn-tool">
                    <i class="far fa-edit"></i>
                  </button>
                </a>
              <?php } ?>
            ...
            <div class="card-body">
              <div class="tab-content p-0">
                ...
                <div class="row mt-3">
                  ...
                  <?php }
                  if ($this->session->userdata('license_expiration_date') !== '0000-00-00') { ?>
                    <div class="col-lg-4 col-6">
                      <div class="inner_details">
                ...
                <?php if ($this->session->userdata('type') == 'admin') { ?>
                  <div class="row mt-3">
                    <?php if (!empty($this->session->userdata('basic_lic_amnt'))) { ?>
                      <div class="col-lg-4 col-6">
                        <div class="inner_details">
                          <label for="">Basic License Amount</label>
                          <h4><?= $this->session->userdata('basic_lic_amnt'); ?></h4>
                        </div>
                      </div>
                    <?php }
                    if (!empty($this->session->userdata('business_lic_amnt'))) { ?>
                      <div class="col-lg-4 col-6">
                        <div class="inner_details">
                          <label for="">Business License Amount</label>
                          <h4><?= $this->session->userdata('business_lic_amnt'); ?></h4>
                        </div>
                      </div>
                    <?php }

                    if (!empty($this->session->userdata('enterprise_lic_amnt'))) { ?>
                      <div class="col-lg-4 col-6">
                        <div class="inner_details">
                          <label for="">Enterprise License Amount</label>
                          <h4><?= $this->session->userdata('enterprise_lic_amnt'); ?></h4>
                        </div>
                      </div>
                    <?php  } ?>
                  </div>
                  ...
                  <div class="row mt-3">
                    ...
                    <div class="col-lg-4 col-6">
                      <div class="inner_details">
                        ...
                        <h4><?= $this->session->userdata('account_type'); ?></h4>
                      </div>
                    </div>
                    <div class="col-lg-4 col-6">
                      <div class="inner_details">
                ...
                <div class="row mt-3">
                  <div class="col-lg-12 col-6 text-primary">
                    <b>PLAN DETAILS:-</b>
                  </div>
                    ...
                    <div class="inner_details">
                      <table class="table table-striped text-secondary">
                        ...
                        <?php for ($i = 0; $i < count($plan_name); $i++) { ?>
                          ...
                          <tr>
                            <td><?= $plan_name[$i]['plan_name']; ?></td>
                            <td><?= $plan_name[$i]['plan_licence']; ?></td>
                            <td><?= $plan_name[$i]['used_licence']; ?></td>
                            <td><?php $tplan = $plan_name[$i]['plan_licence'];
                                $Uplan = $plan_name[$i]['used_licence'];
                                $avlbl = $tplan - $Uplan;
                                echo $avlbl;
                                ?></td>
                            <td><?php $adate = $plan_name[$i]['licence_act_date'];
                                echo date('d-M-Y', strtotime($adate));

                                ?></td>
                            <td><?php $edate = $plan_name[$i]['licence_exp_date'];
                  ...
                  <tbody>
                    ...
                    if (!empty($users_data)) {
                      foreach ($users_data as $users) {
                        $CI   = &get_instance();
                        $pln = $CI->Login_model->yourplanid($users['your_plan_id']);

                    ?>
                        <tr>
                          <td><?php if (isset($pln['plan_name'])) {
                                echo $pln['plan_name'];
                              } ?></td>
        ...
        <form action="#" id="profile_edit" class="form-horizontal" enctype="multipart/form-data" method="post">
          <div class="form-body form-row">
            <div class="col-md-6 mb-3">
              <label>Name</label>
              <input type="text" class="form-control " name="name" id="name" placeholder="Name" value="<?= $this->session->userdata('name'); ?>">
            </div>
            <?php if ($this->session->userdata('type') == 'admin') { ?>
              <div class="col-md-6 mb-3">
                <label>Email</label>
                <input type="email" class="form-control" readonly name="email" id="email" placeholder="Email" value="<?= $this->session->userdata('email'); ?>">
                <span id="email_error"></span>
              </div>
            <?php } else if ($this->session->userdata('type') == 'standard') { ?>
              <div class="col-md-6 mb-3">
                <label>Email</label>
                <input type="email" class="form-control " name="email" id="email" placeholder="Email" value="<?= $this->session->userdata('email'); ?>" readonly>
                <span id="email_error"></span>
              </div>
            <?php } ?>
            <?php if ($this->session->userdata('type') == 'admin') { ?>
              <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control " name="company_contact" id="company_contact" placeholder="Company Contact" value="<?= $this->session->userdata('mobile'); ?>">
                <span id="contact_error"></span>
              </div>
            <?php } else if ($this->session->userdata('type') == 'standard') { ?>
              <div class="col-md-6 mb-3">
                <label>Company Contact</label>
                <input type="text" class="form-control " name="contact_number" id="contact_number" placeholder="Contact Number" value="<?= $this->session->userdata('mobile'); ?>">
                <span id="contact_error"></span>
              </div>
            <?php } ?>
            <?php if ($this->session->userdata('type') == 'admin') { ?>
              <div class="col-md-6 mb-3">
                <label>Country</label>
                <input type="text" class="form-control " name="country" id="country" placeholder="Country" value="<?= $this->session->userdata('country'); ?>">
              </div>
              <div class="col-md-6 mb-3">
                <label>State</label>
                <input type="text" class="form-control " name="state" id="state" placeholder="State" value="<?= $this->session->userdata('state'); ?>">
              </div>
              <div class="col-md-6 mb-3">
                <label>City</label>
                <input type="text" class="form-control " name="city" id="city" placeholder="City" value="<?= $this->session->userdata('city'); ?>">
              </div>
              <div class="col-md-6 mb-3">
                <label>Address</label>
                <input type="text" class="form-control " name="address" id="address" placeholder="Address" value="<?= $this->session->userdata('company_address'); ?>">
              </div>
              <div class="col-md-6 mb-3">
                <label>Zipcode</label>
                <input type="text" class="form-control " name="zipcode" id="zipcode" placeholder="Zipcode" value="<?= $this->session->userdata('zipcode'); ?>">
              </div>
            <?php } ?>
            <?php if ($this->session->userdata('type') == 'admin') { ?>
              <div class="col-md-7 mb-3">
                <label class="upload_logo">Upload Company logo
                  <input type="file" onchange="ValidateSize(this)" name="company_logo" id="company_logo"></label>
              </div>
            <?php } else if ($this->session->userdata('type') == 'standard') { ?>
              <div class="col-md-7 mb-3">
                <label class="upload_logo">Upload Profile Image
      ...
      <div class="modal-body">
        <form action="#" id="add_user_form" class="form-horizontal" enctype="multipart/form-data" method="post" name="add_user_form">
          <div class="form-body form-row">
            ...
            <div class="col-md-6 mb-3">
              <label>Email</label>
              <input type="text" class="form-control " name="standard_email" id="standard_email" placeholder="Email">
              <span id="standard_email_error" style="color: red;float: left;font-size: 14px;"></span>
            </div>
            <div class="col-md-6 mb-3">
              <label>Mobile</label>
              <input type="tel" class="form-control numeric" name="standard_mobile" id="standard_mobile" placeholder="Mobile">
  ...
  $("#update_profile").click(function(e) {
    ...
    $.ajax({
      ...
      success: function(data) {
        console.log(data);
        if (data.status == 200) //if success close modal and reload ajax table
        {
