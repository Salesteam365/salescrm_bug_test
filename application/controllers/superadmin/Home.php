...
class Home extends CI_Controller 
{
  ...
  public function ajax_list()
  {
	...
	public function user_details()
  	{
	    if(!empty($this->session->userdata('loggedsuperadmin_in')))
	    {
	      ...
	      if($type == 'superadmin')
	      {
	      	$admins = $this->Login_model->get_all_loginadmin();
	...
	public function view_userDetails()
  	{
		 if(!empty($this->session->userdata('loggedsuperadmin_in')))
	    {
		    ...
		    $all_admin = $this->Login_model->get_all_loginadmin($view_id);
			$admin = $all_admin->row_array();
			 
			 $standard_user = $this->Login_model->get_all_standarduser($admin['company_email'],$admin['company_name']);
   ...
   public function view_invoiceDetails()
  	{
	    ...
	    else
	    {
	      redirect('superadmin/login');
				...
				//////////////////////////////////////
				///////////////CRM Order List Detais//
				//////////////////////////////////////
	public function crm_order_details()
  	{
	    if(!empty($this->session->userdata('loggedsuperadmin_in')))
	    {
	      ...
	      if($type == 'superadmin')
	      {
	      	$admins['all_admin'] = $this->home_model->get_all_order_list();
			//print_r($admins['all_admin']);die;
			//$data['all_admin'] = $admins->result_array();
		  }
	      
             $this->load->view('superadmin/crm_order_list',$admins);
	    }
	    else
	    {
	      redirect('superadmin/login');
   ...
   public function ajax_list_order_list()
    {
	$product_type="Team365_CRM";
	$list = $this->home_model->get_datatables_order_list($product_type);
	$data = array();
	$no = $_POST['start'];
	$i=1;
    foreach ($list as $post)
    {
	  ...
	  $row[] = $post->user_name;
	  $row[] = $post->company_name;
	  $row[] = $post->company_email;
     if($post->product_name=="Team365_CRM"){
		 $row[] ="Team365 CRM";
	  }else{
           $row[]="";
       }
     if($post->payment_status==0){
		 $row[] =  "<span style='color:red;'>payment failed</span>";
	  }elseif($post->payment_status==1){
		   $row[] = "<span style='color:green;'>Payment successully</span>";
	  }
     $row[] ='<a href="" data-toggle="modal" data-target="#view_admindetails'.$post->id .'">View</a>'; 
    ...
    $output = array(
      "draw" => $_POST['draw'],
      "recordsTotal" => $this->home_model->count_all_order_list(),
      "recordsFiltered" => $this->home_model->count_filtered_order_list($product_type),
      "data" => $data,
    );
    //output to json format
    echo json_encode($output);
				...
				//////////////////////////////////////
				///////////////Invoice Order List Detais//
				//////////////////////////////////////
	public function invoice_order_details()
  	{
	    if(!empty($this->session->userdata('loggedsuperadmin_in')))
	    {
	      ...
	      if($type == 'superadmin')
	      {
			...
			//$data['all_admin'] = $admins->result_array();
             ...
             $this->load->view('superadmin/invoice_order_list',$admins);
	    }
	    else
	    {
	      redirect('superadmin/login');
   ...
   public function ajax_list_invoice_order_list()
    {
	$product_type="Team365_invoice";
	$list = $this->home_model->get_datatables_order_list($product_type);
	$data = array();
	$no = $_POST['start'];
	$i=1;
    foreach ($list as $post)
    {
	  ...
	  $row[] = $post->company_name;
	  $row[] = $post->company_email;
	 // $row[] = $post->user_mobile;
     if($post->product_name=="Team365_invoice"){
		 $row[] ="Team365 invoice";
	  }else{
           $row[]="";
       }
      //$row[] = $post->payment_status; 
	  if($post->payment_status==0){
		 $row[] =  "<span style='color:red;'>payment failed</span>";
	  }elseif($post->payment_status==1){
		   $row[] = "<span style='color:green;'>Payment successully</span>";
	...
	public function change_status()
  	{
        ...
        $data = $this->Login_model->status_update($selected_id ,$status_data );
        if($data == 200){
            ...
            $output ='<!DOCTYPE> <html> <head>
				   ...
				   if($status == 1){
				       ...
				       $this->Login_model->status_update($selected_id ,$active_date );
				   //fetch user email
	...
	public function change_userStatus()
  	{
        ...
        $data = $this->Login_model->statusUser_update($selected_id ,$status_data );
	    print_r($data);
  	}
  ...
  //echo 'hellooosss';
        ...
        if($this->input->post('searchDate') == "u")
        {
          ...
          $data = $this->Login_model->filter_data($searchDate);
  ...
  // echo json_encode($output);
 	...
 	public function profile()
  	{
	    if(!empty($this->session->userdata('loggedsuperadmin_in')))
	    {
	      //$data['user'] = $this->Login_model->getuserdata();
    ...
    public function update_extend()
	{
		  ...
		  $resultdata=$this->Login_model->get_all_loginadmin($id)->row();
  	...
  	public function viewUser()
	{
	    if(!empty($this->session->userdata('email')))
	    {
		  ...
		  //$data = $this->Login_model->get_company_users();
		 // print_r($data);
	    ...
	    if(!empty($this->session->userdata('email')))
	    {
			...
			foreach ($list as $post)
				...
				$output = array(
				  ...
				  "recordsFiltered" => $totalEntry,//$this->Login_model->count_filtered(),
				  "data" 			=> $data,
				);
				echo json_encode($output);
