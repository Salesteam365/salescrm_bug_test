    ...
    public function index()
    {
        if (!empty($this->session->userdata('email'))) {
            ...
            if (check_permission_status('Quotations', 'retrieve_u') == true) {
                $data2 = [];
                $leadStatus = ['Negotiation', 'On Hold', 'Draft', 'Confirmed', 'Closed Lost', 'Closed Won', 'Delivered'];
                ...
                // $data['monthwise'] = $this->Quotation->monthwise_quote();
                $this->db->select('*');
                // $this->db->where('quote.i);
                $this->db->from('quote');
    ...
    public function add_quote()
    {
        if (check_permission_status('Quotations', 'create_u') == true || check_permission_status('Quotations', 'update_u') == true) {
            ...
            if (isset($_GET['opp']) && $_GET['opp'] != "") {
                ...
                if(count($data['record'])!=34){
                    foreach($data['record'] as $keyyy => $ppp){
                        $dsfsd= $this->db->query("SELECT hsn_code,pro_gst FROM product where product_name='".$data['record']['product_name']."'")->row_array();
                        $data['record']['pro_gst'] = $dsfsd['pro_gst'];
                        $data['record']['hsn_code'] = $dsfsd['hsn_code'];
                    }
                }
                else{
                    $dsfsd= $this->db->query("SELECT hsn_code,pro_gst FROM product where product_name='".$ppp['product_name']."'")->row_array();
                    $data['record'][$keyyy]['pro_gst'] = $dsfsd['pro_gst'];
                    $data['record'][$keyyy]['hsn_code'] = $dsfsd['hsn_code'];
                }
                $data['action'] = ['data' => 'add', 'from' => 'opportunity'];
                ...
                $data['action'] = ['data' => 'update', 'from' => 'quotation'];
            } else {
...
<?php
    ...
    public function update_status()
    {
        ...
        $dataArr = ['quote_stage' => $status];
        $data = $this->Quotation->update_status($dataArr, $leadid);
        echo json_encode($data);
    }
    public function getquotationgraph(){
        $data['grouped_data'] = $this->Quotation->getquotegraph();
        $response = [
            'status' => 'success',
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $post) {
            ...
            if ($delete_quote == 1) {
                    ...
                    $row[] = '<input type="checkbox" class="delete_checkbox" onClick="showAction(' . $post->id . ');" name="action_ck" value="'.$post->id.'">';
    ...
    <div class='d-flex align-items-center'>
        ...
        <div>
            ...
            <i style='color:#4e6d7c;font-weight:400;'>".$post->email."</i>
        </div>
    </div>";

    $idquotation =" 
    <span style='color: rgba(140, 80, 200, 1);
    font-weight: 700;'>".ucfirst($post->quote_id)."</span>"
    ;

            // $row[] = $companydetail;
              $row[] = '<a style="text-decoration:none" href="' . base_url() . 'quotation/view_pi_qt/' . $post->id . '">' . $companydetail . '</a>';
            $row[] = $first_row;
          ...
          $quotstatus_backcolors = [
                    'Negotiation' => '#F5B8491A','On Hold' => '#845ADF1A', 'Draft' => '#E6533C1A','Confirmed' => '#a52a2a1A','Closed Lost' => '#967bb61A','Closed Won' => '#cb41541A ','Delivered' => '#ffa07a1A'       
         ];
         $quotstatus_colors = [
            'Negotiation' => '#F5B849',   'On Hold' => '#845ADF', 'Draft' => '#E6533C','Confirmed' => '#a52a2a','Closed Lost' => '#967bb6', 'Closed Won' => '#cb4154 ','Delivered' => '#ffa07a'       
 ];
             $quotstatus="<span class='btn quote_st_bn p-0' style='background:".$quotstatus_backcolors[$post->quote_stage]."; color:".$quotstatus_colors[$post->quote_stage]."'>".$post->quote_stage."</span>";
            $row[]=$quotstatus;
            // $row[]=$post->sub_totalq;
            $row[] = '₹'. IND_money_format((int) $post->sub_totalq);
            $action = '<div class="row" style="font-size: 15px;">';
            if ($retrieve_quote == 1) {
                $action .=
                    '<a style="text-decoration:none" href="' .
                    base_url() .
                    'quotation/view_pi_qt/' .
                    $post->id .
                    '" class="text-success border-right"><i class="far fa-eye sub-icn-opp m-1" data-toggle="tooltip" data-container="body" title="View Quotation Details" ></i></a>';
            }
            if ($update_quote == 1) {
                $action .=
                    '<a style="text-decoration:none" href="' .
            ...
            if ($create_pi == 1 && $PiCount < 1) {
                $action .=
                    '<a style="text-decoration:none" href="' .
                    base_url() .
                    'proforma_invoice/create_newProforma?pg=quotation&qt=' .
                    $post->quote_id .
                    '"  class="text-info border-right">
  ...
  <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink" style="">
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="'.base_url().'quotation/view_pi_qt/' . $post->id . '">Open</a></div>
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="'.base_url().'add-quote/' . $post->id . '">Edit</a></div>
    ...
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Send Reminder by Email</a></div>
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Send Reminder By Whatsapp</a></div>
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Convert to Invoice</a></div>
    ...
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Convert to Salesorder</a></div>
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Convert to Purchase Order</a></div>
    <div class="dropdown-item"><a style="color:black; padding-left:0px;" href="">Duplicate</a></div>
    <div class="dropdown-item copyquote" ><a style="color:black; padding-left:0px;" href="javascript:void(0);" >Copy Quotation link</a></div>
    <input style="display:none;" type="text" class="copyText" value="' . base_url() . 'quotation/view_pi_qt/' . $post->id . '">
    ...
    public function create()
    {
        ...
        if ($validation != 200) {
            ...
            $quote_data = ['track_status' => 'quote'];
            $this->Lead->update_lead_track_status(['opportunity_id' => $opportunity_id], $quote_data);
            $quote_data['stage'] = 'Proposal';
            $this->Opportunity->update_opp_track_status(['opportunity_id' => $opportunity_id], $quote_data);
	...
	*/
    ...
    public function update()
    {
        ...
        if ($validation != 200) {
            ...
            $this->Quotation->update(['id' => $this->input->post('id'), 'session_company' => $this->session->userdata('company_name'), 'session_comp_email' => $this->session->userdata('company_email')], $data);
    ...
    public function check_validation($method = '')
    {
        ...
        if ($this->form_validation->run() == false) {
            return json_encode([
                ...
                'quote_stage' => form_error('quote_stage'),
    ...
    public function view()
    {
        ...
        if ($this->uri->segment(3)) {
            ...
            if (isset($download) && $download == 'dn') {
                $this->dompdf->stream("QUOTATION_" . $id . ".pdf", ["Attachment" => 1]);
            } else {
                $this->dompdf->stream("QUOTATION_" . $id . ".pdf", ["Attachment" => 0]);
            }
        }
    }
    ...
    public function send_email($sd=null)
    {
        // dd($this->input->post());
                                          ...
                                          <table width="100%" border="0" cellspacing="0" cellpadding="0" role="presentation">
                                            <tr>
                                              <td align="center">
                                                <a href="' .
                        $invoiceurl .
                        '" class="f-fallback button" target="_blank">View Quotation Invoice</a>
                                              </td>
                                            </tr>
                                          </table>
                                        </td>
        ...
        if (!$this->email_lib->send_email($orgEmail, $subEmail, $messageBody, $ccEmail, $attach_pdf)) {
            echo "0"; 
            echo "Email sending failed: " . $this->email_lib->get_error_message();
        } else {
      ...
      public function add_mass()
	{
      ...
      $mass_value = $this->input->post('mass_value');
        ...
        // print_r($dataArry);die;
			$mass_data = $this->Quotation->mass_save($mass_id, $dataArry);
			if (!empty($mass_data)) {
				$response = array(
					'success' => true,
