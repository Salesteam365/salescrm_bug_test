	    ...
	    if($type == "admin")
	    {
	      ...
	      foreach ($salesorder as $sales)
	      {
	        $purchaseorder = $this->Reports_model->get_after_discount_po($sales['owner']);
	        {
	            ...
	            $data = array(
	                "owner" => $sales['owner'],
	                "sub_total" => intval($sales['after_discount'])-intval($purchaseorder['after_discount_po'])-intval($sales['total_orc']),
	              );
		    ...
		    if($admin->account_type == 'Trial'){
			    ...
			    if($totalEntry <= $admin->total_licence)
				{
				  ...
				  echo json_encode(array('st' => 200));
					...
					echo json_encode(array('st' => 201, 'show_msg' => '<i class="fas fa-info-circle">You have only '.$admin->total_licence.' licences and you can add only '.($admin->total_licence-1).' users</i>'));
				}
		   }
		endforeach;
                                ...
                                <table border="0" cellpadding="0" cellspacing="0">
                                  <tbody>
                                    <tr>
                                      ...
                                      <td> : </td>
                                      <td>https://allegient.team365.io/</td>
                                    </tr>
                                    <tr>
	...
	public function get_profit_table_product_wise()
  	{
		...
		$getActData = $this->Reports_model->get_Actual_profit_datatables_product_wise($post->saleorder_id,$post->purchaseorder_id);	
	      ...
	      $row[] = $subject;
	      $row[] = $proname;
	      $row[] = intval($post->so_pro_total);
	      $row[] = intval($post->po_total_price);
	    ...
	    $output = array(
	      "draw" 			=> $_POST['draw'],
	      "recordsTotal" 	=> $this->Reports_model->count_all_pro_product_wise(),
	      "recordsFiltered" => $this->Reports_model->count_filtered_pro_product_wise(),
	      "data" 			=> $data,
	    );
	    //output to json format
	    echo json_encode($output);
...
/*
public function get_profit_table_all_User(){
  $data=$this->Reports_model->get_profit_count();
  echo $data;
}*/
...
public function get_act_profit_table_all_User(){
	...
	{	
		...
		{
			$profitTo=intval($calc)-intval($post->total_orc)-intval($post->discount)-intval($post->p_discount);
			$countProfit = $countProfit+$profitTo;
			$arr[]=$post->saleorder_id;
		}
