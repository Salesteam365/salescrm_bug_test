...
class Graph_opp extends REST_Controller {
      ...
      { 
        ...
        }else
        {
          ...
          { 
            ...
            $this->db->where('YEAR(currentdate)',$search_yrs);
          }
          if($this->input->post('profitMoth'))
          { 
            $search_Mnth = $this->input->post('profitMoth');
            $this->db->where('MONTH(currentdate)',$search_Mnth);
          }
          else
          {
          	$this->db->where('currentdate>=',date('Y-m-01'));
          }
     }
