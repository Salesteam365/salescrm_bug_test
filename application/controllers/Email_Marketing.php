...
class Email_Marketing extends CI_Controller
{
	...
	public function send_email()
	{
		...
		if ($img_file != "") {
			...
			$config['file_name'] = time() . "_" . str_replace(' ', '_', $_FILES["images"]['name']);
			$this->load->library('upload');
			$this->upload->initialize($config);

			$uploadedFile = $config['file_name'];
			if (!$this->upload->do_upload('images')) {
				$t_uploadData['status'] = '2';
				$sttatuus = $this->upload->display_errors();
				if($sttatuus=='<p>The uploaded file exceeds the maximum allowed size in your PHP configuration file.</p>'){
					$t_uploadData['msg'] = "The uploaded file exceeds the maximum allowed size.";
				}
				else{
					$t_uploadData['msg'] =  $this->upload->display_errors();
				}
				echo json_encode($t_uploadData); die;
			}
		}
