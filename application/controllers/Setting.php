  ...
  public function addMeeting()
  {
    ...
    if ($addData == '1') {
      ...
      if ($dataRow) {
        ...
        $coun_user = ($userEmail != null) ? count($userEmail) : 0;
        for ($i = 0 ; $i < $coun_user ; $i++) {
...
<html>
  ...
  public function savePrefix()
  {
    ...
    if ( !empty($dataRow) && count($dataRow) > 0) {
      $dataArrMtng = array(
        'module'            => $tablename,
        'prefix_id'         => $prefixid,
      );
    ...
    echo ($dataRow) ? json_encode(["status" => '1']) : json_encode(["status" => '0']);
  ...
  /*END CODE STATE LIST*/
public function fetch_gst_list() {
    try {
       ...
       $query = $this->db
        ->where(['session_comp_email' => $this->session->userdata('email'), 'delete_status' => 1])
        ->order_by('gst_percentage', 'DESC')
        ->get('gst');

        // Fetch the query result as an array and print it for debugging
        $result = $query->result_array();
        
        if (!empty($result)) {
            // Return the GST list as JSON
            header('Content-Type: application/json');
            echo json_encode(['data' => $result]);
        } else {
            // Handle the case where no GST items are found
            echo json_encode(['error' => 'No GST items found']);
        }
    } catch (Exception $e) {
        // Handle exceptions here (e.g., database connection issues)
        // You can log the error or send an appropriate response to the client
