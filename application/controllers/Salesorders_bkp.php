...
class Salesorders extends CI_Controller
{
  ...
  {
    parent::__construct();
    $this->load->helper(array('url','crm_helper'));
...
public function changeStatus(){
  ...
  if(check_permission_status('Sales Order can approve','other')==true){
   ...
   $chngeSt=array('pay_terms_status' =>$stts ,'approved_by' => $approved_by);
   $where=array('id' => $this->input->post('soid'));
...
echo $data;
   ...
   if($data == 1){
	   ...
	   $mailData=implode(",",$arraMail);
		...
		$messageToPoTeam.='<div class="f-fallback">
    		...
    		$messageToPoTeam.='<p>
			Sales Order ID : '.$sales_data->saleorder_id.'
			<br>
			Product : '.str_replace("<br>",", ",$sales_data->product_name).'
			</p>';
			$messageToPoTeam.='For More details <a href="'.base_url().'salesorders/view_pi_so/'.$id.'"> click here</a> or copy this url and paste in your browser<br>
			URL - '.base_url().'salesorders/view_pi_so/'.$sales_data->id.'
			</p>';
    		$messageToPoTeam.='</div>';
	...
	if($permissionSts==true && $workFlowStsStsUser==true){
		 ...
		 $messagetoSoWoner.='<div class="f-fallback">
			...
			$messagetoSoWoner.='For More details <a href="'.base_url().'salesorders/view_pi_so/'.$sales_data->id.'"> click here</a> or copy this url and paste in your browser<br>
			URL - '.base_url().'salesorders/view_pi_so/'.$sales_data->id.'
			</p>';
	...
	if($workFlowStsAdmin==true){	
		 ...
		 $messagetoAdmin.='<div class="f-fallback">
			...
			$messagetoAdmin.='For More details <a href="'.base_url().'salesorders/view_pi_so/'.$sales_data->id.'"> click here</a> or copy this url and paste in your browser<br>
			URL - '.base_url().'salesorders/view_pi_so/'.$sales_data->id.'
			</p>';
    		$messagetoAdmin.='</div>';
    ...
    foreach ($list as $post)
    {
      ...
      if($post->total_percent== '0'){
		 if($invoiceCount>0 && $post->invoice_id!=""){  
			$row[] = '<span class="btn btn-success p-0">Complete</span>';
		 }else{
			$row[] = '<span class="btn btn-secondary p-0">Invoice Pending</span>';
		 }
      }else if($post->total_percent== '100'){
        $row[] = '<span class="btn btn-warning p-0">Pending</span>';
      }else if($post->total_percent > 0 || $post->total_percent < 100)
      {
        $row[] = '<span class="btn btn-primary p-0">In Progress</span>';
      }
	  ...
	  if(isset($post->pay_terms_status) && $post->pay_terms_status==1){
       ...
       $row[] = "<text style='font-size: 12px;'>".$newDate."</text>";
		...
		$action='<div class="row" style="font-size: 15px;">';
				if($retrieve_so==1){
					$action.='<a style="text-decoration:none" href="'.base_url().'salesorders/view_pi_so/'.$post->id.'" class="text-success border-right"><i class="far fa-eye sub-icn-opp m-1" data-toggle="tooltip" data-container="body" title="View Sales Details" ></i></a>';
				}
				if($update_so==1){
					$action.='<a style="text-decoration:none" href="'.base_url().'add-salesorder/'.$post->id.'" class="text-primary border-right">
					<i class="far fa-edit sub-icn-so m-1" data-toggle="tooltip" data-container="body" title="Update Sales Details" ></i></a>';
				}	
				if($create_po==1 && $SOProNameCnt>$POProNameCnt){
					$action.='<a style="text-decoration:none" href="'.base_url().'add-purchase-order?so='.$post->id.'" class="text-info border-right">
					<i class="fas fa-shopping-basket sub-icn-po m-1" data-toggle="tooltip" data-container="body" title="Create Purchase Order" ></i></a>';
				}	
				if($create_Invoice==1 && $invoiceCount<1){
					$action.='<a style="text-decoration:none" href="'.base_url().'add-invoice?so='.$post->id.'" class="text-info border-right">
					<i class="fas fa-file-invoice sub-icn-invoice m-1"  data-toggle="tooltip" data-container="body" title="Create Invoice" ></i></a>';
				}
				if($create_pi==1 && $PiCount<1){		
					$action.='<a style="text-decoration:none" href="'.base_url().'proforma_invoice/create_newProforma?pg=salesorder&qt='.$post->saleorder_id.'"  class="text-info border-right">
					<i class="fas fa-file-invoice-dollar sub-icn-pi m-1"  data-toggle="tooltip" data-container="body" title="Create PI" ></i></a>';
				}
					
					if(isset($post->pay_terms_status) && $post->pay_terms_status==1){
						if($so_approval==1):
							$action.='<a style="text-decoration:none" href="javascript:void(0)" id="textid'.$post->id.'" class="text-success border-right" onclick="approve_entry('."'".$post->id."'".',`'.$post->saleorder_id.'`,`0`,`textid'.$post->id.'`)" ><i class="far fa-thumbs-up text-success m-1"  data-toggle="tooltip" data-container="body" title="Click to disapprove"></i></a>';
						endif;
	      
					}else{
					   if($so_approval==1):
						$action.='<a style="text-decoration:none;" href="javascript:void(0)" id="textid'.$post->id.'"  class="text-primary border-right" onclick="approve_entry('."'".$post->id."'".',`'.$post->saleorder_id.'`,`1`,`textid'.$post->id.'`)"><i class="far fa-thumbs-down text-info m-1"  data-toggle="tooltip" data-container="body" title="Click to approve"></i></a>';
						endif;
					}
				if($delete_so==1){	
					$action.='<a style="text-decoration:none" href="javascript:void(0)" onclick="delete_entry('."'".$post->id."'".',`'.$post->saleorder_id.'`)" class="text-danger">
					<i class="far fa-trash-alt text-danger m-1"  data-toggle="tooltip" data-container="body" title="Delete Salesorder" ></i></a>';
				}
				$action.='</div>';
  ...
  public function getstatusvalue()
  {
	...
	echo json_encode(array('inProgress'=>$inProgress['sub_totals'], 'invoicePend'=>$invoicePend['sub_totals'], 'pening' =>$pening['sub_totals'], 'complete' => $complete['sub_totals']));
  ...
  {  
	...
	if($this->input->post('opportunity_id')=='' || $this->input->post('quote_id')==''){
	  ...
	  if($this->input->post('terms_condition')){
		...
		if($this->input->post('cgst')!=""){ $cgst=implode("<br>", $this->input->post('cgst')); }else{ $cgst=''; }
    ...
    if(isset($data)){  
        ...
        $data = array( 'track_status' => 'salesorder');
        $this->Lead->update_lead_track_status(array('opportunity_id' => $opportunity_id),$data);
		$data['stage'] = 'Closed Won';
        $this->Opportunity->update_opp_track_status(array('opportunity_id' => $opportunity_id),$data);
        $this->load->model('Notification_model');
	    $data=$this->Notification_model->addNotification('salesorders',$id);
		
		add_customer_activity($id, $this->input->post('org_name'),$this->input->post('org_id_act'),$this->input->post('cnt_id_act'),$this->input->post('contact_name'),'customer_sales_order');
  ...
  public function update()
  {
    ...
    else
    {
        ...
        $result = $this->Salesorders->update(array('id' => $this->input->post('id'),'session_company' =>$this->session->userdata('company_name'),'session_comp_email' =>$this->session->userdata('company_email')), $data);
  ...
  public function view()
  {
    if($this->uri->segment(3)){
        ...
        if(isset($download) && $download=='dn'){
    		$this->dompdf->stream("SALESORDER_".$id.".pdf", array("Attachment"=>1));  
    	}else{
    		$this->dompdf->stream("SALESORDER_".$id.".pdf", array("Attachment"=>0));  
    	}
    }
                              ...
                              <tr>
                                <td align="center">
                                  <table width="100%" border="0" cellspacing="0" cellpadding="0" role="presentation">
                                    <tr>
                                      <td align="center">
                                        <a href="'.$invoiceurl.'" class="f-fallback button" target="_blank">View Salesorder Invoice</a>
                                      </td>
                                    </tr>
                                  </table>
                                </td>
